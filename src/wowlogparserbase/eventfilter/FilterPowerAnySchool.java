 ...
 * @author racy
 */
public class FilterPowerAnySchool extends Filter {
     ...
     * @param school The school to match on(from Constants), or FilterPowerSchool.ANY_SCHOOL to match all schools. If two or more schools are chosen then at least one of those schools must match.
     */
    public FilterPowerAnySchool(int school) {
        this.school = school;
    }
    ...
    public boolean filter(BasicEvent e) {
        if (e instanceof PowerEvent) {
            ...
            if ( school != ANY_SCHOOL ) {
                int schoolCheck = school & pe.getSchool();
                if ( schoolCheck == 0 ) {
                    match = false;
                }
            }
